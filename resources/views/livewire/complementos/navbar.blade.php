<div>
    ...
    <header class="bg-gray-dark sticky top-0 z-50 mb-3">
        <div
            class="fixed top-0 left-0 right-0 container mx-auto flex justify-between items-center py-4 bg-slate-700 h-16">
            <!-- Left section: Logo -->
            ...
            <nav class="hidden lg:flex md:flex-grow justify-center">
                <ul class="flex justify-center space-x-4 text-white">
                    ...
                    <li><a href="{{ route('marcas')}}" class="hover:text-cyan-500 font-semibold">Marcas</a></li>
                    <li><a href="{{ url('/producto-shop') }}" class="hover:text-cyan-500 font-semibold">Productos</a>
                    </li>
                    <li><a href="{{ url('/categorias') }}" class="hover:text-cyan-500 font-semibold">Promociones</a>
                    </li>
            ...
            @guest
                <div class="hidden lg:flex items-center space-x-4 relative">
                    <a href="{{ url('/registro') }}"
                       class="bg-primary border border-primary text-white font-semibold px-4 py-2 rounded-full inline-block">Regístrate</a>
                    <a href="{{ url('/login') }}"
                       class="bg-primary border border-primary text-white font-semibold px-4 py-2 rounded-full inline-block">Iniciar
                        Sesión</a>
                </div>
            @endguest
            ...
            @auth
                @if(auth()->user()->hasRole(['Cliente']) == false)
                    ...
                    <div class="hidden lg:flex items-center space-x-4 relative">
                        <a href="/admin"
                           class="bg-primary border border-primary text-white font-semibold px-4 py-2 rounded-full inline-block">Ir
                            al Panel Administrativo</a>
                    </div>
                    <div class="hidden lg:flex items-center space-x-4 relative">
                        <button
                            class="bg-primary border border-primary text-white font-semibold px-4 py-2 rounded-full inline-block"
                            wire:click="logout">Cerrar Sesión
                        </button>
                    </div>
