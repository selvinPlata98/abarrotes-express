        ...
        <div class="main-slider swiper-container">
            <div class="swiper-wrapper">
                ...
                <div class="swiper-slide">
                    <img src="{{asset('/imagen/BannerAbarrotesExpress.jpg')}}" alt="Product 1">
                </div>
            </div>
        </div>
    </section>
                ...
                @forelse ($productos as $producto)
                    @if($producto->disponible == true)
                        <div class="w-full sm:w-1/2 md:w-1/3 lg:w-1/3 xl:w-1/4 px-4 mb-8">
                            <div class="bg-white p-3 rounded-lg shadow-lg text-center h-full flex flex-col">
                                @if(isset($producto->imagenes) && is_array($producto->imagenes) && count($producto->imagenes) > 0)
                                    <img src="{{ url('storage/' , $producto->imagenes[0]) }}" class="w-full h-48 object-cover mb-4 rounded-lg" alt="{{ $producto->imagenes[0] }}">
                                @else
                                    <img src="{{ url('storage/default_image.jpg') }}" class="w-full h-48 object-cover mb-4 rounded-lg" alt="default_image">
                                @endif

                                <a href="{{ route('producto', ['enlace' => $producto->enlace]) }}" class="text-lg font-semibold mb-2 flex-grow">{{$producto->nombre}}</a>
                                    ...
                                    <div class="flex justify-center p-4 border-t border-gray-300 dark:border-gray-700 mt-auto">
                                    <a wire:click.prevent='agregarCarrito({{ $producto->id }})' href="#" class="text-primary flex items-center space-x-2 dark:text-primary hover:text-red-500 dark:hover:text-red-300">
                                        <i class="fa-solid fa-cart-shopping"></i>
                                        <span wire:loading.remove wire:target='agregarCarrito({{$producto->id}})'>Añadir al carrito</span>
                ...
                <div class="relative items-center w-full px-5 py-12 mx-auto md:px-12 lg:px-24 max-w-7xl">
                    <div class="grid w-full grid-cols-1 gap-6 mx-auto lg:grid-cols-3">
                        <div class="flex flex-col p-6 bg-white rounded-xl shadow-lg">
                            ...
                            <p class="flex-grow text-base font-medium leading-relaxed text-gray-txt">Compra con confianza en abarrotes-express, tu tienda en línea donde garantizamos una experiencia satisfactoria</p>
                            <img class="w-full object-cover mb-4 rounded-lg tamanoCard2" src="/imagen/segurida.svg" alt="blog">
                        </div>
                        <div class="flex flex-col p-6 bg-white rounded-xl shadow-lg">
                            <h1 class="mb-4 text-2xl font-semibold leading-none tracking-tighter text-gray-dark lg:text-3xl text-center">Método de pago</h1>
